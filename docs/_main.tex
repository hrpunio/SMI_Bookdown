...
\usepackage{indentfirst} %% umieszczony gdzie indziej nie działa :-(
%%%
...
służy do weryfikacji związku \textbf{przyczyna-skutek}.
...
\textbf{losowo} na dwie grupy: grupa \textbf{eksperymentalna} (\emph{experimental group})
oraz \textbf{grupa kontrolna} (\emph{control group}).
...
ekspozycja (\emph{exposure}) albo leczenie (\emph{treatment})
...
Problem: błąd ekologizmu (\emph{ecological fallacy}). Zależności na poziomie indywidualnym oraz zagregowanym
...
\item
  \textbf{Wprowadzenie}: określenie problemu badawczego i celu badania.
\item
...
\textbf{Współczynnik zgonów} (\emph{death rate}): liczba zgonów w określonym czasie podzielona przez średnią
...
\begin{example}
\textbf{Absolwenci studiów pielęgniarskich w wybranych krajach UE}
...
\text{2,68} dziecka.
Mediana: \text{2,2}. Interpretacja mediany: wartość współczynnika dzietności
...
\textbf{Rozstęp} jest to różnica pomiędzy największą a najmniejszą
...
Wartość rozstępu wynosi \text{5,91} dziecka.
\end{example}
...
\begin{verbatim}
##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
##    77,5    97,0   103,0   102,5   108,5   131,0
\end{verbatim}

Średnia (średnich z próby) ma wartość \text{102,52} kilogramów
a odchylenie standardowe \text{8,84} kilogramów.
...
w eksperymencie (\text{8,84} vs \text{9,14}).

Zauważmy też, że wartość najniższej średniej wyniosła \text{77,5} kilogramów zaś najwyższej \text{131} kilogramów.
Gdybyśmy mieli pecha i wylosowali te skrajnie nieprawdziwe wartości to mylimy się
o \text{25,02} kilogramów
na minus lub \text{28,48} kilogramów na plus.
...
\begin{verbatim}
##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
##   91,22  100,10  103,10  102,99  105,70  116,90
\end{verbatim}

Średnia ma wartość \text{102,99} kilogramów, a odchylenie standardowe
\text{4,16} kilogramów.
Wartość \(s/\sqrt{10}\) jest równa \text{4,09}.

\textbf{Szacujemy średnią na podstawie 40 zawodników pobranych losowo}.

Uwaga: 40 zawodników to \text{6,4}\% całości.
Powtarzamy eksperyment \text{1000} razy.

\begin{verbatim}
##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
##   96,33  101,27  102,81  102,78  104,28  108,85
\end{verbatim}

Średnia jest równa \text{102,78} kilogramów, a odchylenie standardowe \text{2,11} kilogramów.
Wartość \(s/\sqrt{40}\) jest równa \text{2,04}.

Zauważmy też, że wartość najniższej średniej wyniosła \text{96,325} kilogramów zaś najwyższej \text{108,85} kilogramów.
Gdybyśmy mieli pecha i wylosowali te skrajnie nieprawdziwe wartości to mylimy się o \text{6,46} kilogramów
na minus lub \text{6,07} kilogramów na plus. Niewątpliwie wynik znacznie lepszy niż dla próby
dwuelementowej.
...
\begin{verbatim}
##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
##   19,50   38,50   46,00   45,91   53,50   75,00
\end{verbatim}

Średnia średnich z próby ma wartość \text{45,91} lat.
Odchylenie standardowe wyniosło \text{10,71}.
Wartość \(s/\sqrt{2}\) jest równa \text{10,33}.

Wartość najniższej średniej wyniosła \text{19,5} lat zaś najwyższej \text{75} lat.
Gdybyśmy mieli pecha i wylosowali te skrajnie nieprawdziwe wartości to mylimy się
o \text{26,41} lat
na minus lub \text{29,09} lat na plus.
...
\begin{verbatim}
##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
##    32,5    43,1    46,3    46,3    49,3    60,6
\end{verbatim}

Średnia średnich z próby ma wartość \text{46,3} lat.
Odchylenie standardowe wyniosło \text{4,62}.
...
\begin{verbatim}
##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
##   37,92   44,62   46,15   46,15   47,64   53,92
\end{verbatim}

Średnia średnich z próby ma wartość \text{46,15} lat.
Odchylenie standardowe wyniosło \text{2,3}.
...
\begin{verbatim}
##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
##   40,94   45,03   46,29   46,29   47,52   51,71
\end{verbatim}

Średnia średnich z próby ma wartość \text{46,29} lat.
Odchylenie standardowe wyniosło \text{1,8}
Wartość \(s/\sqrt{70}\) jest równa \text{1,75}.

Wartość najniższej średniej wyniosła \text{40,94} lat zaś najwyższej \text{51,71} lat.
Gdybyśmy mieli pecha i wylosowali te skrajnie nieprawdziwe wartości to mylimy się
już tylko o \text{5,35} lat
na minus lub \text{5,43} lat na plus.
...
\item
  Precyzja wnioskowania zwiększa się tym szybciej
...
co w modelu matematycznym będzie opisane jako:
...
Jeżeli wariancja błędu jest duża, to średnia obliczona na podstawie próby
może znacznie się różnić od prawdziwej średniej.
Jeżeli wariancja błędu jest mała, to wartości bardzo różniące
...
\tightlist
\item
  różnica ryzyk (\emph{risk difference}),
\item
  ryzyko względne (\emph{relative risk}), oraz
\item
  iloraz szans (\emph{odds ratio}).
\end{itemize}
...
kolumnę nazywamy \textbf{rozkładami brzegowymi}. Rozkłady brzegowe przestawiają
...
nie zachorowało, a \text{17} zachorowało.
...
Testy U-Manna-Whitneya oraz Kruskala-Wallisa są \textbf{nieparametryczne} bo porównujemy
...
Na poniższym wydruku wartość statystyki testu jest oznaczona jako \texttt{F},
a wartość p symbolem \texttt{p-value}:
...
Konsumpcja mięsa jest mierzona jako średnia roczna konsumpcja w kilogramach w każdym kraju (\emph{per capita} się mówi).
...
nosi nazwę \textbf{metody najmniejszych kwadratów}.
...
wzrost spożycia mięsa o \text{0,001} kg. Przeciętna różnica wartości teoretycznych
...
wynosi \text{0,001}) jest statystycznie istotny.
\end{example}
...
\texttt{CS} (pali) = 1 jeżeli pali, 0 w przeciwnym przypadku;

\texttt{PS} (palił) = 1 jeżeli palił ale nie pali, 0 w przeciwnym przypadku;

\texttt{CD} (pije) oraz \texttt{PD} (pił) \emph{per analogiam} do \texttt{CS}/\texttt{CD}.
...
regresję względem ryzyka dla \(Y\) (czyli \textbf{prawdopodobieństwa}, że \(Y\) przyjmie wartość 1).
...
Model ocenia się
używając jako kryterium \textbf{dewiancję} (\emph{deviance}), której
...
\textbf{Macierz błędów}

Tabela dwudzielna przedstawiająca rozkład wartości prognozowanych i wartości emprycznych
nazywa sie macierzą błędów (\emph{confussion matrix}), por. rysunek \ref{fig:ConfMx}.

\begin{figure}
\includegraphics[width=0.75\linewidth]{./ConfussionMatrix} \caption{Macierz błędów}\label{fig:ConfMx}
\end{figure}

Poszczególne rubryki macierzy błedów zawierają liczbę:
sukcesów zaklasyfikowanych jako „sukces'' (\emph{true positive}, TP),
sukcesów zaklasyfikowanych jako „porażka'' (\emph{false positive}, FP),
porażek zaklasyfikowanych jako „porażka'' (\emph{true negative}, TN)
oraz porażek zaklasyfikowanych jako „sukces'' (\emph{false negative}, FN).

Jakość klasyfikacji ocenia się także za pomocą dwóch wskaźników:
\textbf{czułość} (\emph{sensitivity}) oraz \textbf{swoistość} (\emph{specifity}).
...
\begin{figure}
\includegraphics[width=0.85\linewidth]{./ROCcurve} \caption{Krzywa ROC}\label{fig:ROCcurve}
\end{figure}
...
w grupie \text{392} osób, w tym \text{242}
kobiet (\text{21} przypadków osteoporozy)
oraz \text{150} mężczyzn (\text{5} przypadków osteoporozy).
...
wyniosło \text{0,0663265}. Zatem teoretyczna liczba przypadków osteoporozy
wyniesie\\
\text{0,0663265} \(\cdot\) \text{242} =
\text{16,05} (kobiety) oraz
\text{0,0663265} \(\cdot\) \text{150} =
\text{9,95} (mężczyźni). Dla przypomnienia empiryczne
wartości były równe odpowiednio \text{21} oraz \text{5}.
...
zależność pomiędzy wystąpieniem osteoporozy a płcią, który
...
W tabeli zestawiono wartości parametrów oszacowanego modelu (\texttt{Ocena}),
oceny błędu standardowego parametrów (\texttt{SE}),
ilorazu szans (\texttt{OR}), przedziału ufności parametrów
dla ilorazu szans (CI95) oraz prawdopodobieństwo służące do oceny istotności parametru (\texttt{p}).
...
Można obliczyć, że (teoretyczne) prawdopodobieństwo wystąpienia osteoporozy
wyniosło \text{0,0868} (kobieta) oraz \text{0,0333} (mężczyzna).
Zatem teoretyczna liczba przypadków osteoporozy
wyniesie \text{21} (kobiety) oraz
\text{5} (mężczyźni). Dla przypomnienia empiryczne
wartości były równe odpowiednio \text{21}
oraz \text{5}.

Dewiancja modelu (\text{186,63}) jest istotnie mniejsza
od modelu zerowego (\text{191,32}), ponieważ wartość \(p\) wynosi \text{0,0304}.
Wartość współczynnika pseudo-\(R^2\) McFaddena wyniosła \text{0,0245}
a pseudo-\(R^2\) Nagelkerke wyniosła \text{0,0308}.

Zależność pomiędzy wystąpieniem osteoporozy a płcią (\texttt{genderF}), wiekiem (\texttt{age}) oraz poziomem witaminy D
(\texttt{d}) można opisać następującym równaniem regresji:
...
Dewiancja modelu (\text{121,73}) jest istotnie mniejsza
od modelu zerowego (\text{191,32}), ponieważ wartość \(p\) wynosi \text{0}.
Wartość współczynnika pseudo-\(R^2\) McFaddena wyniosła \text{0,3637}
a pseudo-\(R^2\) Nagelkerke wyniosła \text{0,4212}.

Parametr \texttt{d} okazał się nieistotny statystycznie. Każdy rok (\texttt{age}) zwiększa szanse na osteoporozę
...
Stąd: czułość \text{4} / (\text{22} + \text{4}) = \text{0,1538462};
swoistość \text{362} / (\text{362} + \text{4}) = \text{0,989071}.
Model regresji logistycznej poprawnie klasyfikuje
\text{15,38}\% przypadków osteoporozy
oraz \text{98,91}\% przypadków braku osteoporozy.

Krzywa ROC

\includegraphics[width=0.75\linewidth]{_main_files/figure-latex/unnamed-chunk-75-1}
\end{example}
...
\item
  ...
  (\emph{attitudes}/\emph{emotions}/\emph{intentions}).
\end{itemize}
...
\item
  ...
  mierzyć to co chcemy zmierzyć, bo \textbf{raczej nie należy wymyślać własnych skal};
\item
...
Palą lub paliło \text{62} respondentów
(\text{56,36} \%).
...
\text{13} (czyli \text{68,4}\% wartości maksymalnej).
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{_main_files/figure-latex/rysunek58-1} \caption{Studenci wg stanu psychicznego (\%)}\label{fig:rysunek58}
\end{figure}
...
\item
  Zamiar zmiany miejsca pracy zależy od stażu pracy.
\item
...
W badaniu wzięło udział \text{53} studentek.
...
Zakładamy, że \texttt{zzp} jest objaśniana przez \texttt{sp}, \texttt{sw}, \texttt{kpw}, \texttt{kpp}, \texttt{po}, \texttt{staz} oraz \texttt{roddzial}.
...
Współczynnik zbieżności wynosi \text{41,75}\%.
...
nieistotne. Kolejno należało wyeliminować \texttt{sw}, \texttt{kpp}, \texttt{po}, \texttt{staz} oraz \texttt{roddzial}.
...
Współczynnik zbieżności wynosi \text{33,96}\%.
...
przyjmuje wartość 1 jeżeli \texttt{zzp} jest większe
...
Respondentów, którzy
odejdą jest \text{9} (\text{16,98}\%).

Zakładamy, że \texttt{zp}, podobnie jak w przypadku zamiaru,
jest objaśniana przez \texttt{sp}, \texttt{sw}, \texttt{kpw}, \texttt{kpp}, \texttt{po}, \texttt{staz} oraz \texttt{roddzial}.
...
Dewiancja modelu (\text{30,46}) jest istotnie mniejsza
od modelu zerowego (\text{48,29}), ponieważ wartość \(p\) wynosi \text{0,0127}.
Wartość współczynnika pseudo-\(R^2\) McFaddena wyniosła \text{0,3693}
a pseudo-\(R^2\) Nagelkerke wyniosła \text{0,4778}.
...
Dewiancja modelu (\text{38,45}) jest istotnie mniejsza
od modelu zerowego (\text{48,29}), ponieważ wartość \(p\) wynosi \text{0,0017}.
Wartość współczynnika pseudo-\(R^2\) McFaddena wyniosła \text{0,2037}
a pseudo-\(R^2\) Nagelkerke wyniosła \text{0,2834}.
...
Stąd: czułość \text{0,33}; swoistość \text{0,98}.
Model zp-sp poprawnie klasyfikuje
\text{33,33}\% przypadków odejścia
oraz \text{97,73}\% przypadków pozostania.

Krzywą ROC przedstawiono na rysunku \ref{fig:ROC2}:

\begin{figure}
\includegraphics[width=0.8\linewidth]{_main_files/figure-latex/ROC2-1} \caption{Krzywa ROC modelu zp-sp}\label{fig:ROC2}
\end{figure}
...
\item
  ...
  □ Nie wiem
\item
  Jaki według Ciebie ma wpływ palenie papierosów na narządy zmysłów?
...
też wersja Jamovi365, którą można się posługiwać w przeglądarce i wtedy nie trzeba
...
\item
  ...
  \item
    Analiza zależności między zmiennymi nominalnymi: tablica wielodzielna, test
...
\def\labelenumi{\arabic{enumi}.}
\item
  Klikamy w nazwę zmiennej, którą zamierzamy przekodować. Niech to będzie \texttt{s1}.
...
Część \texttt{jeżeli-prawda} to zwykle wstawienie nowej wartości;
...
ustawiamy kursor na \texttt{s2} w oknie danych. Naciskamy ikonę \texttt{Przekształcenie}.

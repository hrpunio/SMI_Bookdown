...
Wszystkich krajów jest 201. Wartość minimalna współczynnika wynosi 1.22, a wartość
...
Zapis \texttt{(1,1.5{]}} oznacza przedział od 1,0 do 1,5 prz czym dolny koniec nie należy
...
Im więcej przedziałów (mniejsza rozpiętość przedziału) tym histogram jest bardziej szczegółowy co niekoniecznie
...
Dominanta: 1.98. Interpretacja dominanty: najwięcej krajów wykazuje współczynnik dzietności
równy 1.98. Wartość cokolwiek przypadkowa, bowiem krajów wykazujących
współczynnik równy 1.98, jest raptem 4.
...
Współczynnik asymetrii \(g\) wynosi 1.10; Współczynnik
Pearsona wykorzystujący dominantę wynosi 0.55
a wykorzystujący medianę 1.14.
Wreszcie wartość współczynnika opartego o kwartle
wynosi 0.50. Wszystkie współczynniki wskazują
...
Liczba zawodników ataku wyniosła 936.
Przeciętnie zawodnik ataku ważył 92.7 kg.
Wartość mediany wyniosła 92.0 kg (połowa
zawodników ataku ważyła 92.0 kg i mniej).
Wartości pierwszego i trzeciego kwartyla wyniosły odpowiednio 85.5 oraz 99 kg (1/4 zawodników
ataku ważyła 85.5 kg i mniej;
1/4 zawodników ataku ważyła 99 kg i więcej).

Odchylenie standardowe jest równe 10.1 kg (przeciętnie
odchylenie od średniej arytmetycznej wynosi 10.1 kg).
Rozstęp ćwiartkowy wynosi 13.5 kg (rozstęp 50\% środkowych wartości
wynosi 13.5 kg).

Wartość klasycznego współczynnika skośności jest równa 0.34.
Wartość współczynnika skośności opartego o kwartle wynosi 0.04,
a współczynnika skośności Pearsona wykorzystującego medianę wynosi 0.21.
...
Liczba zawodników młyna wyniosła 943.
Średnio zawodnik młyna ważył 112.3 kg.
Wartość mediany wyniosła 112.0 kg (połowa zawodników
młyna ważyło 112 kg i mniej). Wartości
pierwszego i trzeciego kwartyla wyniosły odpowiednio 106 oraz 118 kg (1/4 zawodników
młyna ważyło 106 kg i mniej;
1/4 zawodników młyna ważyło 118 kg i więcej).

Odchylenie standardowe jest równe 9.2 kg (przeciętnie
odchylenie od średniej arytmetycznej wynosi 9.2 kg).
Rozstęp ćwiartkowy wynosi 12 kg (rozstęp 50\% środkowych wartości
wynosi 12 kg).

Wartość klasycznego współczynnika skośności jest równa 0.17.
Wartość współczynnika skośności opartego o kwartle wynosi 0.00,
a współczynnika skośności Pearsona wykorzystującego medianę wynosi 0.11.
...
Średnia (średnich z próby) ma wartość 102.97 kilogramów
a odchylenie standardowe 9.44 kilogramów.
...
w eksperymencie (9.44 vs 9.14).

Zauważmy też, że wartość najniższej średniej wyniosła 76 kilogramów zaś najwyższej 131 kilogramów.
Gdybyśmy mieli pecha i wylosowali te skrajnie nieprawdziwe wartości to mylimy się
o 26.97 kilogramów
na minus lub 28.03 kilogramów na plus.
...
Średnia ma wartość 102.62 kilogramów, a odchylenie standardowe
4.22 kilogramów.
Wartość \(s/\sqrt{10}\) jest równa 4.09.

\textbf{Szacujemy średnią na podstawie 40 zawodników pobranych losowo}.

Uwaga: 40 zawodników to 6.4\%
całości. Powtarzamy eksperyment 1000 razy.
...
Średnia jest równa 102.86 kilogramów, a odchylenie standardowe 2.06 kilogramów.
Wartość \(s/\sqrt{40}\) jest równa 2.04.

Zauważmy też, że wartość najniższej średniej wyniosła 96.5 kilogramów zaś najwyższej 109.425 kilogramów.
Gdybyśmy mieli pecha i wylosowali te skrajnie nieprawdziwe wartości to mylimy się o 6.36 kilogramów
na minus lub 6.57 kilogramów na plus. Niewątpliwie wynik znacznie lepszy niż dla próby
dwuelementowej.
...
Średnia średnich z próby ma wartość 46.61 lat.
Odchylenie standardowe wyniosło 10.23.
Wartość \(s/\sqrt{2}\) jest równa 10.33.

Wartość najniższej średniej wyniosła 18.5 lat zaś najwyższej 74 lat.
Gdybyśmy mieli pecha i wylosowali te skrajnie nieprawdziwe wartości to mylimy się
o 28.11 lat
na minus lub 27.39 lat na plus.
...
Odchylenie standardowe wyniosło 4.68.
...
Średnia średnich z próby ma wartość 46.2 lat.
Odchylenie standardowe wyniosło 2.21.
...
Odchylenie standardowe wyniosło 1.76
Wartość \(s/\sqrt{70}\) jest równa 1.75.

Wartość najniższej średniej wyniosła 41.11 lat zaś najwyższej 51.56 lat.
Gdybyśmy mieli pecha i wylosowali te skrajnie nieprawdziwe wartości to mylimy się
już tylko o 5.13 lat
na minus lub 5.32 lat na plus.
...
Ponieważ wartość \(p\) równa 0.00978 jest mniejsza od każdego zwyczajowo
...
Wartość p równa 0.9295 świadczy o tym, że nie ma istotnych różnic pomiędzy średnimi, co oznacza,
...
0.4405 (różnice są zatem nieistotne; wszystkie rozkłady są identyczne i nie ma zależności)
\end{example}
...
Zatem wśród respondentów było 70\% kobiet i 30\% mężczyzn.
Pracę w szpitalu zadeklarowało 90\% respondentów a w przychodni
10\% respondentów.

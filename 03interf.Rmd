...
na 100-hektarowym polu. Można ściąć wszystkie i zważyć, ale można też ściąć
...
`r wN` rugbystów. Znamy szczegółowe dane odnośnie do wagi każdego
...
`r round(true.mean.w, 2)` kg (`Mean` na wydruku powyżej),
a odchylenie standardowe (*s*) wyniosło `r round(max.err.sd, 2)` kg.
...
Średnia (średnich z próby) ma wartość `r round(mean.s02, 2)` kilogramów,
a odchylenie standardowe `r round(sd.s02, 2)` kilogramów.
Wartość $s/\sqrt{2}$ (odchylenie standardowe podzielone przez pierwiastek kwadratowy 
z liczebności próby) jest równa `r round(max.err.sd/sqrt(2), 2) `. Zauważmy, że ta wartość
jest zbliżona do wartości odchylenia standardowego uzyskanego 
w eksperymencie (`r round(sd.s02, 2)` vs `r round (max.err.sd/sqrt(2), 2) `).
...
Uwaga: 40 zawodników to `r round (40/wN *100, 1)`% całości. 
Powtarzamy eksperyment `r sample.size` razy.
...
Innymi słowy, jeżeli będziemy oceniać wartość prawdziwej średniej na podstawie próby,
...
Znamy szczegółowe dane odnośnie do wieku każdego kandydata,
...
Średnio kandydat miał zatem `r round(true.mean.w, 2)` lat,
a odchylenie standardowe wieku wyniosło `r round(max.err.sd, 2)` lat.

Rozkład znowu jest z jakichś powodów dwumodalny (por. rysunek \@ref(fig:wiekKnR)).
...
prawdopodobne jest np. popełnienie błędu większego niż 10% średniej albo
...
(wynosi ono `r round(nrow( r %>% filter (wiek < 30)) / nrow(r), 2)`%).
...
jest równe: `r round(nrow( r %>% filter (wiek >= 50 & wiek <= 60)) / nrow(r), 2)`%.
...
na podstawie konkretnego estymatora, inaczej mówiąc: jak bardzo mogliśmy się pomylić.
...
Z góry ustalone prawdopodobieństwo nazywa się **poziomem ufności**.
Określenie „95% przedział ufności” oznacza, że 95 razy na 100 będzie on zawierał prawdziwą
wartość parametru (a pięć razy na 100 nie będzie zawierał). Mówimy „95% przedział ufności”
albo „na poziomie ufności równym 95%” przedział zawiera prawdziwą wartość parametru.
...
Formalnie stawiamy **hipotezę**, że różnica średnich wynosi zero.
Jest to tzw. **hipoteza zerowa**. Niezbędne jest także postawienie **hipotezy alternatywnej**,
...
Hipotezy sprawdzamy wykorzystując **statystykę testu** czyli zmienną losową,
...
1. Pechowo trafiła nam się nietypowa próba, która zdarza się rzadko.
...
na podstawie odpowiedniego modelu matematycznego wynosi 0,3,
...
2. Estymator (nieobciążony, zgodny, efektywny): funkcja na wartościach próby która służy
...
6. Poziom ufności: prawdopodobieństwo, że przedział ufności zawiera prawdziwą wartość parametru;

...
Uwaga: ankieta nie jest kolejną metodą statystyczną, lecz techniką zbierania danych.
Wszystkie metody już zostały przedstawione i żadnej nowej nie będzie.

## Jak zacząć badanie?

Przed rozpoczęciem każdego badania, w tym ankietowego,
należy zastanowić się nad trzema sprawami: 

1. Co chcemy ustalić? 

2. Jakie dane są nam potrzebne, żeby ustalić, to co planujemy?
...
Najlepiej jakąś zależność. Na przykład: stres a wypalenie zawodowe;
...
zawodowym to niewątpliwie powinniśmy zmierzyć stres i wypalenie.  Jak
dotąd jest banalnie prosto. Problem zaczyna się w momencie odpowiedzi na
pytanie **jak**?

### Mierzenie twardych faktów vs mierzenie przekonań

Możemy pytać w ankiecie o dwie rzeczy:

* o **fakty** (wiek, staż, zawód, tętno, przebyte choroby);

* o **przekonania**, **wartości**, **postawy**; 
...
bywa czasami mocno arbitralne.

### Skala Likerta

Przykładowo chcemy się dowiedzieć, czy i jak bardzo respondenci boją się COVID-19. 

W najprostszej wersji po prostu pytamy: 
**Czy pan/pani boi się COVID-19?** i dajemy 
...
z odpowiedzią,
czy boi się bardziej-bardziej, czy jednak bardziej-bardziej-bardziej.
...
wielokrotny. 
W naukach podstawowych mierzymy (np. linijką) parę razy, a wynik uśredniamy,
co daje pomiar bardziej precyzyjny. Tutaj pytamy parę razy o to samo, co
ma dać podobny efekt (mniejszy średni błąd pomiaru). 
Taka seria pytań nosi też nazwę skali albo **inwentarza**.
Nie pytamy zatem **Czy pan/pani boi się COVID-19?**, tylko zadajemy serię
pytań o strach względem COVID-19.
...
mianem refleksyjnego (co jest kalką angielskiego słowa *reflexive*). Na rysunku \@ref(fig:modelP)
...
aspekt** czynnika. Ktoś może być profesorem filozofii, nie mieć pracy i kiepskie
dochody. 
Tylko jeden z trzech aspektów podwyższa mu SES. Inny przykład to
świetnie zarabiająca prostytutka bez matury.
...
w czynniku formatywnym pominiemy indykator, to popełniamy bardzo duży błąd, bo pomijamy
...
wartość indykatorów. Jeżeli indykatory są mierzone za pomocą skali Likerta, to
wartością czynnika będzie suma (albo średnia) wartości rang. W skali strachu przed COVID-19 ten, kto się najbardziej
boi powinien odpowiedzieć 7 razy **zdecydowanie tak**, co odpowiada 
sumie 35 rang (jeżeli rangujemy od 1 do 5). 
Osoba, która wcale się nie boi, będzie zaś miała 7.

Małym utrudnieniem mogą być **pytania odwrócone**. Jeżeli pytamy
o strach przed COVID-19 i w każdym pytaniu, jak bardzo ktoś się boi, albo jak
...
przekonania** (strach, samoskuteczność, wypalenie zawodowe, stres czy satysfakcja):
...
wśród
studentów studiów zaocznych Powiślańskiej Szkoły Wyższej (PSW)
oraz zweryfikowanie wpływu wybranych czynników warunkujących ten nałóg.
...
otrzymywał minusowy punkt (aby nie opłacała się strategia zaznaczenia wszystkich odpowiedzi).
...
```{r rysunek51, fig.cap="Studenci według wyniku testu na znajomość szkodliwości palenia", echo=F, out.width=ex.out.width}
...
```{r rysunek52, fig.cap="Studenci według statusu palenia (%)", out.width=ex.out.width}
s0.status <- s1 %>%
  select (status) %>%
  group_by(status)%>%
  summarize(n=n())%>%
...
```{r rysunek54, fig.cap="Studenci według stażu pracy (%)", out.width=ex.out.width}
s0.staz <- s1 %>%
  select (staz) %>%
  group_by(staz)%>%
  summarize(n=n())%>%
...
Pracę w szpitalu zadeklarowało `r s1.praca.S.prop`% badanych, a w przychodni 
-- `r 100 - s1.praca.S.prop`%.
...
Pali lub paliło `r pala_lub_palili` respondentów 
(`r round(pala_lub_palili/ P.total.N *100, 2)`%).
Żeby stwierdzić, czy to jest dużo, czy mało, to na przykład można by porównać to
...
Nie jest, czego dowodzi wysoka wartość p (`r round(chi_test$p.value, 4)`).
...
Nie, wskazuje na to wysoka wartość p (`r round(chi_test$p.value, 4)`).
...
wykazują się wyższą od mężczyzn wiedzą na temat szkodliwości palenia.
...
do odrzucenia hipotezy o identyczności rozkładów  na poziomie p = 0,05. Nie ma różnicy
...
do odrzucenia hipotezy o identyczności rozkładów na poziomie p = 0,05. Nie ma
...
## Depresja i jej uwarunkowania wśród studentów Powiślańskiej Szkoły Wyższej

### Cel

Celem jest ustalenie, czy depresja jest istotnym problemem wśród
studentów studiów zaocznych Powiślańskiej Szkoły Wyższej (PSW)
oraz zweryfikowanie wybranych czynników warunkujących depresję.
...
wypełnione ankiety. Średnia wartość testu Becka wyniosła `r round(s0.mean, 3)` (odchylenie
standardowe `r round(s0.sd, 3)`).

Rozkład ankietowanych według wyniku testu Becka przedstawiono na rysunku \@ref(fig:rysunek56).


```{r rysunek56, fig.cap="Studenci według wyniku testu Becka", out.width=ex.out.width}
...
```{r rysunek57, fig.cap="Studenci według stażu pracy (%)", out.width=ex.out.width}
##
s0.staz <- s0 %>%
  select (staz) %>%
  group_by(staz)%>%
...
Rozkład studentów według stanu psychicznego przedstawiono na wykresie \@ref(fig:rysunek58).

```{r rysunek58, fig.cap="Studenci według stanu psychicznego (%)", echo=F, out.width=ex.out.width.x}
s0.dep <- s0 %>%
  select (Depresja) %>%
  group_by(Depresja)%>%
  summarize(n=n())%>%
...
Jeżeli depresję mierzymy na (oryginalnej) skali liczbowej, to
...
Jeżeli depresję mierzymy na skali liczbowej, to można zastosować test Welcha lub test Manna-Whitneya.
...
6. Zmiana miejsca pracy zależy od satysfakcji z pracy, satysfakcji z wynagrodzenia,
...
Badanie ankietowe wśród studentek studiów zaocznych PSW przeprowadzono w 2023 i 2024 roku.
...
o staż oraz o to,
czy pracują na oddziale ratunkowym/intensywnej terapii (zmienna `roddzial`).
...
W badaniu wzięły udział `r sample.size` studentki.
...
Tylko zmienne `sp` oraz `kpw` okazały się istotne.
...
Żeby zademonstrować przykład wykorzystania regresji logistycznej, przyjmijmy,
że jeśli ktoś bardzo chce
...
przyjmuje wartość 1, jeżeli `zzp` jest większe lub
...
a pseudo-$R^2$ Nagelkerke -- `r round(nagelkerke.1, 4)`.
...
a pseudo-$R^2$ Nagelkerke -- `r round(nagelkerke.2, 4)`.
...
3. Popełnię samobójstwo, gdy będzie odpowiednia sposobność.
...
19. Jaki według Ciebie ma wpływ palenie papierosów na narządy zmysłów? 
...
**Przywiązanie do organizacji/PO** (`PO-1`–`PO-18`), składające się z następujących aspektów:
...
24. Zakład pracy, w którym pracuję, ma dla mnie duże znaczenie. (PO-2)

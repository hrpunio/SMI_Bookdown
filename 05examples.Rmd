...
`r P.total.q3` (czyli `r round(P.total.q3/19 *100, 1)` %).
...
```{r rysunek58, fig.cap="Studenci wg stanu psychicznego (%)", echo=F, out.width=ex.out.width}
s0.dep <- s0 %>%
  select (Depresja) %>%
  group_by(Depresja)%>%
  summarize(n=n())%>%
...
przyjmuje wartość 0 jeżeli `zzp` jest większe

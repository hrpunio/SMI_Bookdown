...
Przejście od definicji koncepcyjnej do definicji operacyjnej bywa czasami mocno, hmm... arbitralne.
...
kable(s0.pl, col.names = c('płeć', 'średnia', 'n'), booktabs = TRUE)
...
s0.st <- s1 %>%
  group_by(staz) %>%
  summarise(m = mean(P.total), n=n())
kable(s0.st, col.names = c('staż', 'średnia', 'n'), booktabs = TRUE)
...
s0.sw <- s1 %>%
  group_by(status) %>%
  summarise(m = mean(P.total), n=n())
kable(s0.sw, col.names = c('status', 'średnia', 'n'), booktabs = TRUE)
...
kable(s0.staz, col.names = c('staż', 'średnia', 'n'), booktabs = TRUE)

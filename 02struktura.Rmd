...
t3 <- kable(osoby.L, col.names = c('liczba osób', 'liczba gospodarstw', '%'), booktabs = TRUE)
t3
```
...
Współczynnik asymetrii $g$ wynosi `r sprintf("%.2f", skewness2018)`; Współczynnik
Pearsona wykorzystujący dominantę wynosi `r sprintf("%.2f", skewness2018WS_d)`
a wykorzystujący medianę `r sprintf("%.2f", skewness2018WS_m)`.
Wreszcie wartość współczynnika opartego o kwartle
wynosi `r sprintf("%.2f", skewness2018WS_q)`. Wszystkie współczynniki wskazują
...
Wartość mediany wyniosła `r sprintf("%.1f", median.b)` kg (połowa
...
Odchylenie standardowe jest równe `r sprintf("%.1f", sd.b)` kg (przeciętnie
...
Wartość klasycznego współczynnika skośności jest równa `r sprintf ("%.2f", skewness.b)`.
Wartość współczynnika skośności opartego o kwartle wynosi `r  sprintf ("%.2f", skewness.b_q)`,
a współczynnika skośności Pearsona wykorzystującego medianę wynosi `r  sprintf ("%.2f", skewness.b_m)`.
...
Wartość mediany wyniosła `r sprintf("%.1f", median.f)` kg (połowa zawodników
...
Odchylenie standardowe jest równe `r sprintf("%.1f", sd.f)` kg (przeciętnie
...
Wartość klasycznego współczynnika skośności jest równa `r sprintf ("%.2f", skewness.f)`.
Wartość współczynnika skośności opartego o kwartle wynosi `r  sprintf ("%.2f", skewness.f_q)`,
a współczynnika skośności Pearsona wykorzystującego medianę wynosi `r  sprintf ("%.2f", skewness.f_m)`.
...
#| iqr        | `r iqr.b`    | `r iqr.f`    |
#| g          | `r iqr.b`    | `r iqr.f`    |
miara <- c('średnia', 'mediana', 'odchyl.st', 'iqr', 'skośność')

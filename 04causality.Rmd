...
```{r testy, out.width="75%", fig.cap = "Testowanie istotości różnicy pomiędzy średnimi"}
...
```{r KMNK, out.width="80%", fig.cap = "Metody statystycznej weryfikacji zależności pomiędzy zmiennymi"}
...
Współczynnik zbieżności wynosi `r sprintf ("%.2f", lmr * 100)`%. 
Kolumna `Beta`, której wcześniej nie było, zawiera standaryzowane
...
nie ma wartości standaryzowanej (oznaczone jako `NA` czyli *not available*),

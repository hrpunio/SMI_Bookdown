...
`r round(narciarze.x[3,2], 2)`% a nie zachorowania
`r round(narciarze.x[3,1], 2)`%. Jest przyznajmy całkiem **zdroworozsądkowym założeniem**
...
Konsumpcja mięsa jest mierzona jako średnia roczna konsumpcja w kilogramach w każdym kraju (*per capita* się mówi).
...
nazywa sie macierzą błędów (*confussion matrix*), por. rysunek \@ref(fig:ConfMx).
...
```{r ROCcurve, out.width="85%", fig.cap = "Krzywa ROC"}

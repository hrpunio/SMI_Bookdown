...
Związki między zmiennymi mogą być albo **funkcyjne** – wartościom jednej zmiennej 
...
a chorobą nowotworową (skutek), wiekiem a prawdopodobieństwem zgonu z powodu COVID-19 itd.?
...
```{r metodyAZ, out.width="99%", fig.cap = "Metody statystycznej weryfikacji zależności pomiędzy zmiennymi"}
knitr::include_graphics("./DiagramMetod.pdf")
```
...
Jeden narciarz nie dokończył eksperymentu. Historia milczy, dlaczego tak się stało :-)
...
Na chłopski rozum: jeżeli witamina C **nie działa**, to powinien
...
Jeżeli $r_e$ oznacza ryzyko w grupie eksperymentalnej (*test group*, grupa narażona, *exposed group*),
a $r_k$ w grupie kontrolnej (*control group*, grupa nienarażona, *unexposed group*), 
...
ryzyka; $RR > 1$ oznacza zwiększenie; $RR = 1$ oznacza brak zależności.
...
### Przedziały ufności dla ryzyka względnego i ilorazu szans
...
albo zwiększa szanse na niezachorowanie od $1/0,25 = 4$ do $1/0,9 \approx 1,1$. Żeby
to zabrzmiało ładnie i po polsku: zwiększa szanse na niezachorowanie od 10% do 300%.
...
rownames(narciarze.table) <- c('grupa C', 'grupa P', 'razem')
kable(narciarze.table, col.names = c('zdrowy', 'katar', 'razem'), booktabs = TRUE)
```

Taka tabela składa się z wierszy i kolumn.
...
kolumnę nazywamy **rozkładami brzegowymi**. Rozkłady brzegowe przestawiają
rozkłady każdej zmiennej osobno.
...
nie zachorowało, a `r narciarze[1,2]` zachorowało. 
...
narciarze.table.p <- as.data.frame.matrix(addmargins(narciarze.p))
rownames(narciarze.table.p) <- c('grupa C', 'grupa P', 'razem')
kable(digits=2, narciarze.table.p, col.names = c('zdrowy', 'katar', 'razem'), booktabs = TRUE)
```

Narciarze, którzy brali witaminę C oraz nie zachorowali, stanowią
`r round(narciarze.table.p[1,1], 2)`%
...
rownames(narciarze.x) <- c('grupa C', 'grupa P', 'razem')

kable(digits=2, narciarze.x, col.names = c('zdrowy', 'katar', 'razem'), booktabs = TRUE)
```

Otrzymaliśmy ryzyka zachorowania na katar (lub nie zachorowania). Ryzyko
zachorowania dla całej grupy wynosi
`r round(narciarze.x[3,2], 2)`%, a ryzyko niezachorowania
to `r round(narciarze.x[3,1], 2)`%. Jest, przyznajmy, całkiem **zdroworozsądkowym założeniem**
...
`r  round(narciarze.x[3,2], 2)`% narciarzy, którzy
brali witaminę C, a widzimy, że zachorowało
jedynie `r round(narciarze.x[1,2], 2)`%.

Na oko księgowego witamina C działa (bo są różnice), ale dla statystyka liczy się to,
...
sprawdzić,
że jest to `r round(narciarze.table[3,1] * narciarze.table[1,3] / narciarze.table[3,3], 3)`):

```{r, message=FALSE, echo=FALSE}
chi_skiers <- chisq.test(narciarze)
narciarze.expected <- chi_skiers$expected
narciarze.ex.table <- as.data.frame.matrix(addmargins(narciarze.expected))

rownames(narciarze.ex.table) <- c('grupa C', 'grupa P', 'razem')
chipv <- chi_skiers$p.value
##chipv.txt <- sprintf("%f", chipv)
kable(digits=3, narciarze.ex.table, col.names =  c('zdrowy', 'katar', 'razem'),  booktabs = TRUE)
```
...
Za pomocą testu chi-kwadrat obliczamy prawdopodobieństwo wystąpienia
tak dużych lub większych różnic. Wynosi ono `r round(chipv, 4)`.
...
katarem a braniem witaminy C, pomylimy się pięć razy lub raz na 100.
...
zweryfikowaliśmy fakt, czy obie zmienne są niezależne, czy nie.
...
sklasyfikować wszystkich badanych w następujący sposób:
...
smoke.table <- as.data.frame.matrix(addmargins(smoke))
rownames(smoke.table) <- c('current', 'former', 'never', 'razem')

kable(smoke.table, col.names =  c('High', 'Low', 'Middle', 'razem'), booktabs = TRUE)
```

Uwaga: status-społeczno-ekonomiczny, to miara prestiżu używana w socjologii
(można na Wikipedii doczytać, co to dokładnie jest).

Tym razem tabela składa się z trzech wierszy i trzech kolumn (ostatni wiersz/kolumna się
nie liczą, bo to są sumy – rozkłady brzegowe).
...
Sum <- margin.table(smoke,2) /smoke.total
smoke.x <- cbind(rbind(smoke.p, Sum), c(1,1,1)) * 100
rownames(smoke.x) <- c('current', 'former', 'never', 'razem')

kable(smoke.x, digits=2, col.names = c('High', 'Low', 'Middle', 'razem'), booktabs = TRUE)
```
...
smokers.ex.table <- as.data.frame.matrix(addmargins(smokers.expected))
rownames(smokers.ex.table) <- c('current', 'former', 'never', 'razem')

chipv <- chi_smokers$p.value
##chipv.txt <- sprintf("%f", chipv)
kable(smokers.ex.table, col.names =  c('High', 'Low', 'Middle', 'razem'), booktabs = TRUE)
```

Wartość prawdopodobieństwa dla testu chi-kwadrat określająca, że przy założeniu
niezależności obu zmiennych tak duża różnica między liczebnościami rzeczywistymi a idealnymi 
(porównaj stosowne tabele wyżej) jest dziełem przypadku wynosi `r round(chipv, 4)`.
Jest to prawdopodobieństwo tak małe, że statystyk odrzuca założenie o niezależności
statusu i palenia (myląc się w przybliżeniu `r round(chipv, 4)` ≈ raz na tysiąc).

:::

## Zmienna liczbowa i zmienna nominalna

Obserwujemy wartości zmiennej liczbowej **w grupach** określonych przez wartości zmiennej nominalnej,
...
W postaci diagramu ze strzałkami przedstawiono to na rysunku \@ref(fig:testy).

```{r testy, out.width="80%", fig.cap = "Testowanie istotności różnicy pomiędzy średnimi"}
knitr::include_graphics("./TestFlowChart.pdf")
```
...
Omawiając wynik, należy podać wartość $t_k$ oraz p. Statystyka testu może się
różnie nazywać i być oznaczana różnym symbolem,
np.: t (test t Welcha), U (test U Manna-Whitneya).

Testy $t$ Welcha oraz ANOVA są **parametryczne**, porównujemy średnie w grupach.
Testy U-Manna-Whitneya oraz Kruskala-Wallisa są **nieparametryczne**, bo porównujemy
...
s0$staz <- factor(s0$staz, levels = c('<06', '07-12', '13-18', '>19'))
...
Średnie różnią się 
o `r round(s0.mp.szpital - s0.mp.przychodnia, 2)`. 
...
Ponieważ wartość $p$ równa `r pval.mp` jest mniejsza od każdego zwyczajowo
przyjmowanego poziomu istotności (np. 0,05 albo 0,1), hipotezę, że średnie w obu grupach są równe należy odrzucić.
...
zmiennej. Dla **poziomu depresji** wygląda jak na poniższym rysunku:
...
są identyczne wynosi `r  pval.u`. Różnica jest zatem istotna na każdym zwyczajowo
...
dobry zwyczaj nakazuje podawać tę wartość omawiając wynik testu, dlatego to czynimy.
...
Sprawdzamy, czy średnie we wszystkich grupach są równe.
...
Wartość p równa `r round(p.anova, 4)` świadczy o tym, że nie ma istotnych różnic pomiędzy średnimi, co oznacza,
...
`r round(pval, 4)` (różnice są zatem nieistotne; wszystkie rozkłady są identyczne i nie ma zależności).
...
Konsumpcja mięsa jest mierzona jako średnia roczna konsumpcja w kilogramach na mieszkańca w każdym kraju (*per capita*).
Dochód zaś jest mierzony jako średni dochód w tysiącach dolarów na mieszkańca w każdym kraju (*per capita*). 
Dane dotyczą roku 2013.

```{r message=FALSE, echo=FALSE, out.width="90%"}
## Dane FAO/WorldBank
meatCons <- read.csv(file='meatCons_vs_GDP.csv',sep=';',header=T) |>
  mutate (gdp2013 = gdp2013 / 1000)
...
który można wysnuć z wykresu rozrzutu.
...
można to zapisać w postaci następującej formuły:

$$\mathrm{cov} (xy) = \frac{1}{n} \left( (x_1 - \bar x)\cdot (y_1 - \bar y)  + ... +
(x_n- \bar x)\cdot (y_n - \bar y) \right)$$

Kowariancja zależy od rozproszenia (im większe, tym większa), 
ma też dziwną jednostkę (jednostkaX · jednostkaY) oraz zależy 
od wybranych skal (np. tony vs gramy).

Z tych powodów do pomiaru związku pomiędzy cechami używa się
...
S. Mohammadi i inni badali zależność pomiędzy wiekiem, poziomem edukacji, szczęściem a stanem zdrowia.
...
h0 <- read.csv("iran_happiness.csv", sep = ',', dec = ".",  header=T, na.string="NA" ) %>%
  ...
  na.omit()
round(cor(h0, use = "complete.obs"), 4)
```
...
Przypominamy też, że wartość średnia **dobrego estymatora** powinna wynosić zero (bo wtedy nie ma błędu systematycznego)
...
uzyskane **metodą najmniejszych kwadratów** mają obie właściwości.
...
Można udowodnić, że bez względu na to, czy punkty na wykresie układają się
...
współczynnik zbieżności jest wyrażony w procentach), tym lepiej.
...
## sprintf("%s",format(1.5,decimal.mark=",", digits=3))
...
Wzrost wagi zawodnika o 1 kg
skutkuje przeciętnie większym wzrostem o `r round(coeff_b, 3)` cm. Współczynnik determinacji
wynosi `r round(lmr * 100, 2)`%. 
...
Poniższe równanie opisuje zależność pomiędzy dochodem narodowym na głowę (tys USD *per capita*)
...
Każdy 1000 USD *per capita* więcej dochodu narodowego (GDP) oznacza przeciętny 
wzrost spożycia mięsa o `r round(coeff_b, 3)` kg.  Przeciętna różnica wartości teoretycznych
...
wynosi `r round(coeff_b, 3)`) jest statystycznie istotny.
...
s0 <- read.csv("RC_Health_Care_samples.csv", sep = ';', header = T ) %>%
  mutate (
      ...
      CS = case_when(smoking.status == 1 ~ 1, TRUE ~ 0),
      PS = case_when(smoking.status == 2 ~ 1, TRUE ~ 0),
      CD = case_when(drinking.status == 1 ~ 1, TRUE ~ 0),
      PD = case_when(drinking.status == 2 ~ 1, TRUE ~ 0)
  )
...
lm_ci_txt <- sprintf ("%s %s", format(lm.2.ci[,1], decimal.mark=",", digits=3), format(lm.2.ci[,2], decimal.mark=",", digits=3) )

## zestawienie tabelaryczne wyników
lm.2.coef.df <- tibble::rownames_to_column(lm.2.coef, "Parametr") %>%
  mutate(std=lm.2.std,  ci=lm_ci_txt)
...
lm_ci_txt <- sprintf ("%s %s", format(lm.3.ci[,1], decimal.mark=",", digits=3), format(lm.3.ci[,2], decimal.mark=",", digits=3) )

## zestawienie tabelaryczne wyników
lm.3.coef.df <- tibble::rownames_to_column(lm.3.coef, "Parametr") %>%
  mutate(std=lm.3.std,  ci=lm_ci_txt)
...
Należy każdą zamienić na dwie zmienne binarne, według schematu:
...
```{r}
lm.1 <- lm(data=s1, SBP ~ BMI + age + genderF + CS + PS + CD + PD );
lmsum1 <- summary(lm.1)
##lmsum1
##lm.beta(lm1)
lmr <- summary(lm.1)$r.squared
...
Współczynnik zbieżności wynosi `r round(lmr * 100, 2)`%. Tylko dwie na siedem zmiennych
...
```{r}
lm.3 <- lm(data=s3, SBP ~ BMI + age + genderF + CS + PS + CD + PD );
lmsum3 <- summary(lm.3)
lmr <- summary(lm.3)$r.squared
lmr7 <- lmr
##lmsum1
...
Współczynnik zbieżności wynosi `r round(lmr * 100, 2)`%. Zwiększenie
...
jaka wielkość efektu ma **znaczenie praktyczne**, i na tej podstawie określamy
...
```{r}
lm.3 <- lm(data=s3, SBP ~ BMI + age + genderF + CS + PS + CD );
lmsum3 <- summary(lm.3)
lmr <- summary(lm.3)$r.squared
##lmsum1
##lm.beta(lm1)
...
```{r}
lm.3 <- lm(data=s3, SBP ~ BMI + age + genderF + CS + CD );
lmsum3 <- summary(lm.3)
lmr <- summary(lm.3)$r.squared
##lmsum1
##lm.beta(lm1)
...
współczynnika zbieżności ostatecznego modelu wynosi `r round(lmr * 100, 2)`%.
...
Jeżeli zmienna $Y$ jest zmienną **dwuwartościową**, czyli taka, która przyjmuje tylko dwie 
...
Ale zamiast szacować regresję $Y$ względem $X$ (lub $X$-ów), można szacować
...
wzrośnie o $b_i$ (przy założeniu, że pozostałe zmienne $X$ mają
...
prostego modelu pomiędzy wystąpieniem osteoporozy a płcią, który ma postać:
...
W modelu zerowym prawdopodobieństwo osteoporozy jest identyczne dla
...
jest wyższe od przyjętej **wartości granicznej** ($p_g$) lub jej równe, to zakładamy „sukces”,
jeżeli tak nie jest, to zakładamy „porażkę”. 
Wartość $p_g$ jest ustalana albo
arbitralnie, albo na podstawie jakieś dodatkowej (pozastatystycznej) informacji.
Domyślnie przyjmuje się zwykle $p_g = 0,5$, co oznacza, że
wartości $p \geq 0,5$ zostaną zamienione na „sukces”,
a wartości $p < 0,5$ na „porażkę”.

**Macierz błędów**

Tabela dwudzielna przedstawiająca rozkład wartości prognozowanych i wartości empirycznych 
nazywa się macierzą błędów (*confussion matrix*), por. rysunek \@ref(fig:ConfMx).

```{r ConfMx, out.width="75%", fig.cap = "Macierz błędów"}
knitr::include_graphics("./ConfussionMatrix.pdf")
```

Poszczególne rubryki macierzy błędów zawierają liczbę:
sukcesów zaklasyfikowanych jako „sukces” (*true positive*, TP),
sukcesów zaklasyfikowanych jako „porażka” (*false positive*, FP),
porażek zaklasyfikowanych jako „porażka” (*true negative*, TN)
oraz porażek zaklasyfikowanych jako „sukces” (*false negative*, FN).

Jakość klasyfikacji ocenia się także za pomocą dwóch wskaźników:
**czułości** (*sensitivity*) oraz **swoistości** (*specifity*). 

Czułość to odsetek sukcesów zaklasyfikowanych jako „sukces” 
($\mathrm{TP}/(\mathrm{TP}+\mathrm{FN})$).
Swoistość to  odsetek porażek zaklasyfikowanych 
jako „porażka” ($\mathrm{TN}/(\mathrm{TN} + \mathrm{TP})$).

**Ocena dopasowania: krzywa ROC**

Czułość oraz swoistość zależą od prawdopodobieństwa granicznego.
Im wyższa
...
czułości oraz swoistości dla wybranych wartości granicznych.
...
knitr::include_graphics("./ROCcurve.pdf")
```
...
sample.by.gender.M <- vd0 |> filter(genderF == 0) |> summarise(n = n()) |>
  unlist() |> unname()
osteoporosis.by.gender.K <- vd0 |> filter (osteoporosis ==1) |>
  filter(genderF ==1) |> summarise(n =n()) |> unlist() |> unname()
osteoporosis.by.gender.M <- vd0 |> filter (osteoporosis ==1) |>
  filter(genderF ==0) |> summarise(n =n()) |> unlist() |> unname()

```

Al Zarooni A.A.R i inni badali wpływ różnych czynników,
...
w grupie `r sample.size` osób, w tym `r sample.by.gender.K` 
kobiet (`r osteoporosis.by.gender.K` przypadków osteoporozy)
oraz `r sample.by.gender.M` mężczyzn (`r osteoporosis.by.gender.M` przypadków osteoporozy).
...
Zacznijmy od modelu zerowego, tj. takiego, w którym szansa
wystąpienia osteoporozy jest takie sama bez względu na wielkości innych zmiennych.
Odpowiada to następującemu równaniu:

$$\ln(o) = `r round(intercept, 5)`0$$

Można obliczyć, że (teoretyczne) prawdopodobieństwo wystąpienia osteoporozy
wyniosło `r prob0`. Zatem teoretyczna liczba przypadków osteoporozy 
wyniesie     
`r prob0` $\cdot$ `r sample.by.gender.K` = 
`r round(prob0 * sample.by.gender.K, 2)` (kobiety) oraz
`r prob0` $\cdot$ `r sample.by.gender.M` = 
`r round(prob0 * sample.by.gender.M, 2)` (mężczyźni). Dla przypomnienia empiryczne
wartości były równe odpowiednio `r osteoporosis.by.gender.K` oraz `r osteoporosis.by.gender.M`.
...
Można obliczyć, że (teoretyczne) prawdopodobieństwo wystąpienia osteoporozy
wyniosło `r round(probF, 4)` (kobieta) oraz `r round(probM,4)` (mężczyzna). 
Zatem teoretyczna liczba przypadków osteoporozy 
wyniesie `r round(probF * sample.by.gender.K, 2)` (kobiety) oraz
`r round(probM * sample.by.gender.M, 2)` (mężczyźni). Dla przypomnienia empiryczne
wartości były równe odpowiednio `r osteoporosis.by.gender.K` 
oraz `r osteoporosis.by.gender.M`.

Dewiancja modelu (`r round(glm.1$deviance, 2)`) jest istotnie mniejsza 
od modelu zerowego (`r round(glm.0$deviance, 2)`), ponieważ wartość $p$ wynosi `r round(pchisq.glm.1, 4)`.
...
W tabeli zestawiono wartości parametrów oszacowanego modelu, 
oceny błędu standardowego parametrów,
ilorazu szans, przedziału ufności parametrów 
dla ilorazu szans oraz prawdopodobieństwo służące do oceny istotności parametru.
...
od modelu zerowego (`r round(glm.0$deviance, 2)`), ponieważ wartość $p$ wynosi `r round(pchisq.glm.2, 4)`.
...
Stąd: czułość  `r table[4]` / (`r table[3]` + `r table[4]`) = `r czulosc`; 
swoistość  `r table[1]` / (`r table[1]` + `r table[2]`) = `r swoistosc`.
Model regresji logistycznej poprawnie klasyfikuje 
`r round(czulosc * 100, 2)`% przypadków osteoporozy
oraz `r round(swoistosc * 100, 2)`% przypadków braku osteoporozy.
...
## Przypadek specjalny: co najmniej dwie zmienne porządkowe
...
* współczynnik Pearsona: `r round(rpm, 2)`;

* współczynnik Spearmana: `r round(rsm, 2)`.
...
Nie ma sensu liczenie współczynnika korelacji rang w przypadku, kiedy obie
...
wynosi `r round(rsm, 2)` (umiarkowana korelacja).

Czy ta wartość jest istotnie różna od zera? Jest na to stosowny
test statystyczny, który sprowadza się do określenia, jakie jest
prawdopodobieństwo otrzymania $r_s$ = `r round(rsm, 2)` przy założeniu, że 
prawdziwa wartość $r_s$ wynosi 0. Otóż w naszym przykładzie

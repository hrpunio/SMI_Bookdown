...
kable(digits=2, narciarze.table.p, booktabs = TRUE)
```
...
kable(digits=2, narciarze.x, booktabs = TRUE)
```
...
kable(s0.mp, col.names = c('m-pracy', 'średnia', 'n'), booktabs = TRUE)
```
...
s0.staz <- s0 %>%
  group_by(staz) %>%
  summarise(m = mean(P), n=n())
kable(s0.staz, col.names = c('staż (kategoria)', 'średnia', 'n'), booktabs = TRUE)
```
...
`PS` (*past smoker*/kiedyś palił) = 1 jeżeli palił ale nie pali, 0 w przeciwnym przypadku

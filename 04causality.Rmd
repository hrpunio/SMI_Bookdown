...
kable(digits=3, narciarze.ex.table, booktabs = TRUE)
```
...
kable(smoke.table, booktabs = TRUE)
```
...
kable(smoke.x, col.names = c('High', 'Low', 'Middle', 'Sum'), booktabs = TRUE)
```
...
##chipv.txt <- sprintf("%f", chipv)
kable(smokers.ex.table, booktabs = TRUE)
```
...
Testy U-Manna-Whitneya oraz Kruskala-Wallisa są **nieparametryczne** porównujemy
...
kable(lm.3.coef.df, row.names = F, digits=3,
      col.names = c('Zmienna', 'B', 'SE', 'z', 'p', 'Beta', 'CI'), booktabs = TRUE )
```

...
Wyraz „statystyka” ma wiele znaczeń. Są np. **statystyki zgonów** albo **statystyki
...
przedmiotem badania statystycznego, np. zgony w Polsce w roku 2022.
...
np. część zgonów w Polsce w roku 2022.
...
np. *Zgony mieszkańców Kwidzyna w roku 2022*.

Zwróćmy uwagę, że *Zgony w mieście Kwidzyn w roku 2022* to nie to samo
(ktoś może być mieszkańcem tego miasta,
a umrzeć gdzieś indziej i/lub ktoś może nie być
kwidzynianinemi umrzeć w tym mieście).
...
Wiek oraz numer dnia są liczbami, zaś płeć i przyczyna symbolem.
...
Średnie wyniki w
grupie studentów pijących dużo kawy były wyższe niż w grupie pijącej jej mało.
...
więcej czasu na naukę, pili w tym czasie kawę (np. żeby nie zasnąć). 
...
niefluoryzujących wodę. Okazało się, że przeciętnie współczynnik ten był wyższy
...
Mogę pić kawę lub jej nie pić, a wynikiem będzie ocena. Oczywiście
nie mogą zrobić tych dwóch rzeczy naraz.
...
Można przypuszczać, że oprócz kawy na wynik egzaminu mają wpływ np. wrodzone predyspozycje
...
Następnie zalecamy studentom w **grupie eksperymentalnej** picie 1 l kawy dziennie,
a studentom w **grupie kontrolnej** podajemy 1 l brązowej wody o smaku i zapachu kawy :-).
Średnie wyniki w
grupie studentów pijących 1 l kawy okazały się wyższe niż w grupie pijącej kolorową wodę.
...
w badaniach opisowych nie ma grupy kontrolnej.
...
Problemem są koszty (np. choroby rzadkie wymagają ogromnych kohort).
...
Problem 1.: błąd pamięci (*recall bias*) pacjenci – zwłaszcza zdrowi – słabo pamiętają
fakty, które miały miejsce lata temu.
Problem 2.: trudności z **dopasowaniem** grupy kontrolnej (łatwiej powiedzieć niż zrobić).

Badania **prospektywne**: od przyczyny do skutku (*cohort*); badanie **retrospektywne**:
od skutku do przyczyny (*case-control*).

Badanie **przekrojowe** (*cross-sectional study*): podobne do badania kliniczno-kontrolnego, ale podział
na grupy kontrolną i eksperymentalną następuje po wybraniu próby, np. pacjentki po badaniu mammograficznym
są dzielone według wyniku badania.

Badania opisowe to zwykle badania przekrojowe, w tym **badania ankietowe**. W badaniu opisowym
można ustalić zależność (korelacyjną) czegoś z czymś, nie da się jednak ustalić związku przyczyna-skutek.
...
Badanie **ekologiczne**: badanie (przekrojowe) zależności pomiędzy wielkościami **zagregowanymi**, a nie **indywidualnymi** danymi.
...
w kraju występują duże różnice w dochodach (np. w USA), to przeciętnie dochód jest wysoki, ale jest
...
B. Neal i inni zastosowali eksperyment kontrolowany do zbadania
...
chlorku sodu, zaś grupa badana używała soli tradycyjnej czyli zawierającej
...
Michael Simon, Angelika Kümmerling, Hans-Martin Hasselhorn; Next-Study Group
...
w drugiej domenie. To teoria zapewne zapożyczona z obszaru nauk o zarządzaniu.
Ten konflikt jest mierzony odpowiednią skalą pomiarową składającą się z pięciu pytań.
Czynnikami, które mają powodować WHC, są: czas pracy,
...
Badano 27603 osoby.
Narzędziem badawczym jak się łatwo domyśleć była ankieta,
...
rak szyjki macicy dotyczy  kobiet, a nie wszystkich ludzi.
...
**Współczynnik śmiertelności** (*case fatality rate*): liczba zgonów z powodu
X w określonym czasie (dzień, tydzień, rok) podzielona
...
Mamy w tym zakresie trzy możliwości:
